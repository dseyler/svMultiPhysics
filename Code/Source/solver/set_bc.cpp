...
void calc_der_cpl_bc(ComMod& com_mod, const CmMod& cm_mod, const SolutionStates& solutions)
{
  ...
  // Analytical-Jacobian path: skip both FD-probe loops below. Reads
  // dP/dQ for each Coupled BC directly from svZeroD's persistent
  // SparseLU factorization (one back-solve per face, no extra 0D Newton
  // solve). Only valid when useSvZeroD is true; the older
  // bc.cplBCptr-style Neu branch is not needed here because the
  // pipelines that use cplBC_I_analytical exclusively use the newer
  // iBC_Coupled path.
