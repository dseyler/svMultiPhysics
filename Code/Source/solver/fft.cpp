    ...
    double delay)
{
  ...
  // Evaluate the curve at com_mod.time shifted back by `delay` (default 0 -> the
  // current time). Used for per-element active-stress activation delay.
  double time = com_mod.time - delay;
  #ifdef debug_ifft
  dmsg << "time: " << time;
