...
void struct_3d_linear(ComMod &com_mod, CepMod &cep_mod, const int eNoN,
  ...
  // Stack storage behind non-owning views; see struct_3d.
  double fb_buf[3];
  ...
  double F_buf[9];
  double vx_buf[9] = {};
  Array<double> F(3,3,F_buf), vx(3,3,vx_buf);
  vx = 0.0;
  F  = 0.0;
  F(0,0) = 1.0;
  F(1,1) = 1.0;
  ...
  // Exactly sized and heap allocated: struct_3d_linear runs once per element,
  // not once per quadrature point, so one allocation here is already 1/nG of
  // what the per-point path cost. A stack buffer would have to be sized for the
  // largest supported element and its frame measurably outweighs that saving.
