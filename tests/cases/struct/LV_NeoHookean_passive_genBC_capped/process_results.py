...
cap_reference_surface = os.path.join(sim_folder, 'mesh/mesh-surfaces/endo_cap.vtp')
...
# Compute cap dVdt from simulation results
(t_dVdt_3D, dVdt_3D_cap) = calc_dVdt_struct(start_time, end_time, step, results_folder, cap_reference_surface)
# Convert volume to cm^3/s
dVdt_3D_cap = np.array(dVdt_3D_cap) # cm/s * m^2
dVdt_3D_cm3_cap = dVdt_3D_cap * (100)**2 # cm^3/s

# Compute lumen volume from AllData file
vol_0D_cm3 = calc_volume_struct_genBC(os.path.join(sim_folder, alldata_file), 3, t_3D)
# Add on initial volume
vol_0D_cm3 += vol_3D_cm3[0] # cm^3

# Compute flow rate = dVdt from AllData file
dVdt_0D_cm3_endo = calc_volume_struct_genBC(os.path.join(sim_folder, alldata_file), 5, t_dVdt_3D)
dVdt_0D_cm3_cap = calc_volume_struct_genBC(os.path.join(sim_folder, alldata_file), 6, t_dVdt_3D)
dVdt_0D_cm3 = dVdt_0D_cm3_endo - dVdt_0D_cm3_cap


# Compute lumen pressure at iterations in t
pressure = calc_pressure_struct_genBC(os.path.join(sim_folder, "AllData"), 4, t_3D) # dynes/cm^2
...
# Plot 3D and 0D dVdt
fig, ax = plt.subplots()
ax.plot(dVdt_3D_cm3, label = 'dVdt_3D', color = 'blue')
ax.plot(dVdt_0D_cm3, label = 'dVdt_0D', linestyle = '--', color = 'blue')
ax.plot(dVdt_3D_cm3_cap, label = 'dVdt_3D_cap', color = 'red')
ax.plot(dVdt_0D_cm3_cap, label = 'dVdt_0D_cap', color = 'red', linestyle = '--')
