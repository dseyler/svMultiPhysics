...
c Block for reading the data from phasta
     ...
     2   QNeumann(nNeumannSrfs,4))
      ...
      OPEN(2, FILE='QinPout.dat', STATUS='UNKNOWN', ACCESS='APPEND');
      DO i=1,nNeumannSrfs
         READ (1) Qi(i), Qf(i)
...
c        AB 2/16/23: After every FINDF, we should add offset to X? This is because
c        we are solving a differential algebraic equation with RK4
         X = Xo
...
c        AB 2/16/23: Update offset and set to Xo for Neumann surfaces. 
c        AB 4/19/23: Commenting out because I think this causes numerical issues
c        This enforces the algebraic equations at t+dt
c         CALL FINDF(t+dt, Xo, f(:,4), QNeumann(:,4),
c     2       PDirichlet(:,4) )
c         DO i=1, nNeumannSrfs
c            Xo(srfToXPtr(i))  = offset(srfToXPtr(i))
c         END DO
...
c        Print out pressure from genBC to svFSI for debugging
     ...
     2      'Pressure from genBC: ', offset(srfToXPtr(i))
         WRITE(2,*) 'BC i = ', i, 
     2      'Pressure to genBC: ', offset(srfToXPtr(i))

c        AB 2/16/23: Commenting this part out
c        AB 4/19/23: Uncommenting
...
c     2      + offset(srfToXPtr(i)))*pConv
      ...
      END DO
c     Print out valve states
      PRINT*, 'Valve resistance AV = ', Xprint(6)
      WRITE (2,*) 'Valve resistance AV = ', Xprint(6)

      PRINT*, 'Valve resistance SL = ', Xprint(7)
      WRITE (2,*) 'Valve resistance SL = ', Xprint(7)
...
c        Write a new line in QinPout.dat
         OPEN(2, FILE='QinPout.dat', STATUS='UNKNOWN', ACCESS='APPEND');
         WRITE(2,*) ''
